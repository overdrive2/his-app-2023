...
trait DateTimeHelpers
{
    public function getCurrentDate() {
        $data = DB::connection('his')->select("SELECT CURRENT_DATE");
        return collect($data)->first()->current_date;
    }

    public function getTomorrowDate() {
        $data = DB::connection('his')->select("SELECT (CURRENT_DATE + interval '1 day') as d ");
        return collect($data)->first()->d;
    }

    public function getCurrentSTM() {
        $data = DB::connection('his')->select("SELECT CURRENT_TIMESTAMP::char(19)");
        return collect($data)->first()->current_timestamp;
    }

    public function getCurrentTime() {
        $data = DB::connection('his')->select("select CURRENT_TIME::time(0)");
        return collect($data)->first()->current_time;
    }

    public function getWsaeDate($date) {
        return collect(DB::connection('his')->select("select (cast(date_trunc('week', '".$date."'::date) as date)"
